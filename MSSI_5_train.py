...
model.load_weights("./model/MSSI_5-void-weights4.h5")
...
checkpoint = ModelCheckpoint("./model/MSSI_5_void_val_loss_best_weights.h5",
...
history = model.fit(x=[x_train, mask_train], y=y_train, validation_data=(
    [x_val, mask_val], y_val), shuffle=True, epochs=30, callbacks = [checkpoint])

model.save_weights("./model/MSSI_5-void-weights5.h5")
with open("./model/MSSI_5-void-weights5", 'wb') as file_pi:
