...
def MSSI_Model_Low():
    ...
    output_img = Conv2D(64, (5, 5), padding='same', kernel_initializer='he_normal')(input_img)
    output_mask = Conv2D(1, (5, 5), padding='same', kernel_initializer=tf.keras.initializers.Ones(), trainable = False)(input_mask)
    ...
    output_img = Conv2D(64, (5, 5), padding='same', kernel_initializer='he_normal')(output_img)
    output_mask = Conv2D(1, (5, 5), padding='same', kernel_initializer=tf.keras.initializers.Ones(), trainable = False)(output_mask)
    mask_ratio = tf.keras.layers.Lambda(lambda tensor: 9 / (tensor + 1e-8))(output_mask)
    output_mask = tf.keras.layers.Lambda(lambda tensor: K.clip(tensor, 0, 1))(output_mask)
    mask_ratio = multiply([output_mask, mask_ratio])
    output_img = multiply([mask_ratio, output_img])
...
def MSSI_Model_High():
    ...
    output_img = Conv2D(1, (5, 5), padding='same', kernel_initializer='he_normal')(output_img)
