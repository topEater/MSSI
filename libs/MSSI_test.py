...
def MSSI_Model_Low():
    ...
    def convlayer():
        return Conv2D(64, (3, 3), padding='same', kernel_initializer='he_normal')
    
    conv = convlayer()

    output_img = conv(input_img)
    weights = conv(input_mask)
    ...
    model = Model(inputs = [input_img, input_mask], outputs = [output_img, weights])
    return model
...
def MSSI_Model():
    ...
    output_img, weights = low_part([input_img, input_mask])
